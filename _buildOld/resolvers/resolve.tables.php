...
if ($transport->xpdo) {
    $modx =& $transport->xpdo;
    $modelPath = MODX_CORE_PATH . 'components/tickets2/model/';

    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        case xPDOTransport::ACTION_INSTALL:
        case xPDOTransport::ACTION_UPGRADE:
            $modx->addPackage('tickets2', $modelPath);
            ...
            if ($modx instanceof modX) {
                $modx->addExtensionPackage('tickets2', '[[++core_path]]components/tickets2/model/');
            }
            break;

        case xPDOTransport::ACTION_UNINSTALL:
            if ($modx instanceof modX) {
                $modx->removeExtensionPackage('tickets2');
            }
            break;
    }
}
