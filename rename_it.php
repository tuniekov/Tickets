...
function copyFiles($sourcePath, $destinationPath) {
    ...
    // Копируем содержимое из исходной папки в целевую
    ...
    foreach ($files as $file) {
        $dest = $destinationPath . DIRECTORY_SEPARATOR . $files->getSubPathName();
        if ($file->isDir()) {
            if (!file_exists($dest)) {
    ...
    foreach ($items as $item) {
        ...
        if (is_dir($new_path)) {
            ...
            if ($item == 'home.class.php') {
                $content = str_replace($replace[0] . 'ManagerController', 'MyExtraManagerController', $content);
            }
