Tickets2.grid.Section = function (config) {
    config = config || {};

    Ext.applyIf(config, {
        url: Tickets2.config.connector_url,
        baseParams: {
            action: 'Tickets2\\Processors\\Mgr\\Ticket\\GetList',
        ...
        standalone: false,
        stateful: true,
        stateId: 'tickets2-grid-state',
    });
    Tickets2.grid.Section.superclass.constructor.call(this, config);
    this.getStore().sortInfo = {
        field: 'id',
        direction: 'DESC'
    };
};
Ext.extend(Tickets2.grid.Section, MODx.grid.Grid, {
    ...
    getColumns: function (config) {
        return [{
            ...
            renderer: function (value, metaData, record) {
                return Tickets2.utils.ticketLink(value, record['data']['id'])
            },
            id: 'pagetitle'
        }, {
            ...
            sortable: true,
            renderer: function (value, metaData, record) {
                return Tickets2.utils.ticketLink(value, record['data']['section_id'], true)
            },
    ...
    getTopBar: function (config) {
        ...
        if (!config.standalone) {
            tbar.push({
                text: '<i class="icon icon-plus"></i> ' + _('ticket_create'),
    ...
    createTicket: function () {
        var createPage = MODx.action
            ...
            : 'resource/create';
        location.href = 'index.php?a=' + createPage + '&class_key=Tickets2\\Model\\Ticket&parent=' + MODx.request.id + '&context_key=' + MODx.ctx;
    },
    ...
    ticketAction: function (method) {
        ...
        MODx.Ajax.request({
            url: Tickets2.config.connector_url,
            params: {
                action: 'Tickets2\\Processors\\Mgr\\Ticket\\Multiple',
                method: method,
    ...
    unpublishTicket: function () {
        this.ticketAction('unpublish');
    },

    duplicateTicket: function () {
...
Ext.reg('tickets2-grid-tickets2', Tickets2.grid.Section);