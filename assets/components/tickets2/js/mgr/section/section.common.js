Tickets2.panel.SectionSettings = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        forceLayout: true,
        stateful: true,
        stateId: 'tickets2-section-settings-tabs',
        stateEvents: ['tabchange'],
        getState: function () {
            return {activeTab: this.items.indexOf(this.getActiveTab())};
        },
        items: this.getTabs(config),
        bodyCssClass: 'tickets2-section-settings tickets2'
    });
    Tickets2.panel.SectionSettings.superclass.constructor.call(this, config);
};
Ext.extend(Tickets2.panel.SectionSettings, MODx.VerticalTabs, {
    ...
    getTabs: function (config) {
        ...
        tabs.push({
            ...
            items: this.getTickets2Settings(config)
        });

        tabs.push({
            title: _('tickets2_section_tab_ratings'),
            hideMode: 'offsets',
            anchor: '100%',
            layout: 'form',
            defaults: {
            ...
            items: this.getTickets2Ratings(config)
        });
    ...
    getContentField: function (config) {
        return {
            ...
            value: config.record && config.record.content
                ...
                : MODx.config['tickets2.section_content_default'],
    ...
    getTickets2Settings: function (config) {
        var items = [{
            html: _('tickets2_section_tab_tickets2_intro'),
            border: false,
            bodyCssClass: 'panel-desc',
        }];

        var tmp = {
            template: {xtype: 'tickets2-children-combo-template', anchor: '50%'},
        ...
        for (var i in tmp) {
            if (tmp.hasOwnProperty(i)) {
                items.push(Ext.apply({
                        ...
                        name: 'properties[tickets2][' + i + ']',
                        hiddenName: 'properties[tickets2][' + i + ']',
                        fieldLabel: _('tickets2_section_settings_' + i),
                        id: 'tickets2-settings-children-' + i,
                        value: config.record.properties['tickets2'][i],
                        listeners: config.listeners,
                        anchor: '25%'
                    },
                    tmp[i]
                ...
                items.push({
                    xtype: 'label',
                    html: _('tickets2_section_settings_' + i + '_desc'),
                    cls: 'desc-under'
                });
            }
        }
    ...
    getTickets2Ratings: function (config) {
        return [{
            html: _('tickets2_section_tab_ratings_intro'),
            border: false,
        }, {
            layout: 'column',
            items: [{
    ...
    _getRatingField: function (name, config, properties) {
        ...
        return [
            Ext.apply({
                ...
                fieldLabel: _('tickets2_section_rating_' + name),
                id: 'tickets2-settings-rating-' + name,
            ...
            {
                xtype: 'label',
                html: _('tickets2_section_rating_' + name + '_desc'),
                cls: 'desc-under'
            }
        ];
    },
...
Ext.reg('tickets2-section-tab-settings', Tickets2.panel.SectionSettings);